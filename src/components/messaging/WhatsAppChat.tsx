...
const WhatsAppChat = ({ agentName, agentPhone, propertyTitle, isOpen, onClose }: WhatsAppChatProps) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center sm:items-center p-4">
      ...
      {/* Chat Window */}
      <div className="relative w-full max-w-md bg-slate-900 rounded-2xl overflow-hidden shadow-2xl border border-white/10 animate-scale-in">
        {/* Header - Semkat branded */}
        ...
        {/* Chat Body */}
        <div className="p-4 space-y-4 bg-slate-900 min-h-[200px]">
          {/* WhatsApp branding notice */}
          <div className="text-center">
            <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-green-500/10 border border-green-500/20">
              ...
              <span className="text-green-400 text-xs font-medium">Chat via WhatsApp</span>
            </div>
          </div>

          {/* Message Preview */}
          <div className="bg-white/5 rounded-xl p-3 border border-white/10">
            <p className="text-white/70 text-sm mb-2">Your message:</p>
            <p className="text-white text-sm">{message}</p>
          </div>

          {/* Message Input */}
          <div className="space-y-2">
            <label className="text-white/70 text-sm">Edit your message:</label>
            <textarea
              value={message}
              onChange={(e) => setMessage(e.target.value)}
              className="w-full min-h-[100px] bg-white/5 border border-white/20 rounded-lg p-3 text-white placeholder:text-white/40 focus:border-semkat-orange focus:ring-1 focus:ring-semkat-orange resize-none"
